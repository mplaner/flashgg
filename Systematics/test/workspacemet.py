...
from flashgg.Systematics.SystematicDumperDefaultVariables import defaultVariables,minimalVariables,minimalHistograms,minimalNonSignalVariables,defaultVariables,defaultHistograms,systematicVariables
import os
...
#"root://eoscms.cern.ch//eos/cms//store/group/phys_higgs/cmshgg/sethzenz/flashgg/ReMiniAOD-03Feb2017-2_5_1/2_5_1/DoubleEG/ReMiniAOD-03Feb2017-2_5_1-2_5_1-v0-Run2016H-03Feb2017_ver3-v1/170214_121150/0000/myMicroAODOutputFile_1.root"
...
vertex_variables =["gVz              := tagTruth().GVz()",
                 "dVz                := tagTruth().Vz()",
                 "dV0z               := tagTruth().V0z()"]
vh_variables   =["hasZ               := tagTruth().associatedZ",
                 ...
                 "dPhi_tagged_Vmet   :=abs(deltaPhi(met().corPhi, tagTruth().Vphi))",
                 "dPhi_pf_Vmet       :=abs(deltaPhi(metPhi, tagTruth().Vphi))",
                 "Vpt                := tagTruth().Vpt",
                 "Vphi               := tagTruth().Vphi",
                 ]
met_uncertainty=["diphoton_mass      := diPhoton().mass",
                 "isVtx0             :=isVtx0",
                 "dipho_phi          := diPhoton().phi()",
                 "pfMET_corPt        := met().corPt",
                 "pfMET_corPhi       := met().corPhi",
                 "pfMET_dPhi         := abs(deltaPhi(met().corPhi, diPhoton().phi()))",
                 "pfMET_corSumEt     := met().corSumEt",
                 "jet_pt            := ? jet().isNonnull() ? jet().pt():0",
                 "jet_phi            := ? jet().isNonnull() ? jet().phi():0",
                 "dPhi_pfMetJet      := ? jet().isNonnull() ? abs(deltaPhi(met().corPhi, jet().phi())):0",
                 "dPhi_diphoJet      := ? jet().isNonnull() ? abs(deltaPhi(diPhoton().phi(), jet().phi())):0",
                 "pfMET_unc          := uncUnc",
                 "pfMET_jer          := jerUnc",
                 "pfMET_jen          := jenUnc",
                 "pfMET_pho          := phoUnc",
                 ...
                 "leadPhotonID       := leadingView().phoIdMvaWrtChosenVtx()",
                 "subPhotonID        := subLeadingView().phoIdMvaWrtChosenVtx()",
                 "dPhi_taggedMetJet  := ? jet().isNonnull() ? abs(deltaPhi(metPhi, jet().phi())):0",
                 "tagged_met_dPhi    := abs(deltaPhi(metPhi, diPhoton().phi()))"
                 ]
...
for tag in tagList: 
  ...
  if tagName == "VHMetTag":
      if  customize.processId.count("mc_higgs"):
          currentVariables = defaultVariables + vh_variables + met_uncertainty
      else:
          currentVariables = defaultVariables + met_uncertainty
  elif tagName == "ZHLeptonicTag" or tagName == "VHHadronicTag":
      currentVariables = defaultVariables
  else:
      currentVariables = defaultVariables
